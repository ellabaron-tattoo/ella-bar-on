...
function inlineOpts(){try{var modal=document.querySelector('.cta-modal');var main=document.querySelector('.cta-main');var opts=modal?modal.querySelectorAll('.cta-opt'):[];if(!opts.length||!main)return;if(document.querySelector('.cta-opts-inline'))return;var wrap=document.createElement('div');wrap.className='cta-opts-inline';wrap.style.cssText='display:flex;flex-direction:column;gap:12px;max-width:520px;width:100%;margin:18px auto 32px;padding:0 20px;box-sizing:border-box;direction:rtl;';var hint=document.createElement('div');hint.style.cssText='text-align:center;font-size:13px;color:rgba(240,232,212,0.65);margin-bottom:4px;letter-spacing:0.5px;';hint.textContent='▼  בחר/י את האפשרות המתאימה';wrap.appendChild(hint);opts.forEach(function(o){var c=o.cloneNode(true);c.style.cssText+=';box-shadow:0 8px 24px rgba(0,0,0,0.4),0 2px 6px rgba(0,0,0,0.3)!important;border:1.5px solid rgba(240,232,212,0.35)!important;transition:all .25s ease!important;cursor:pointer;position:relative;';wrap.appendChild(c);});main.parentElement.insertBefore(wrap,main.nextSibling);main.style.display='';main.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();wrap.scrollIntoView({behavior:'smooth',block:'center'});},true);var fl=document.querySelector('.cta-float');if(fl){fl.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();wrap.scrollIntoView({behavior:'smooth',block:'center'});},true);}}catch(e){}}
