...
function inlineOpts(){try{var modal=document.querySelector('.cta-modal');var main=document.querySelector('.cta-main');var opts=modal?modal.querySelectorAll('.cta-opt'):[];if(!opts.length||!main)return;if(document.querySelector('.cta-opts-inline'))return;main.style.display='none';var wrap=document.createElement('div');wrap.className='cta-opts-inline';wrap.style.cssText='display:flex;flex-direction:column;gap:12px;max-width:600px;width:100%;margin:8px auto 32px;padding:0 16px;box-sizing:border-box;direction:rtl;';var hint=document.createElement('h2');hint.style.cssText='text-align:center;font-family:"Assistant","Heebo",sans-serif;font-size:clamp(26px,6.5vw,54px);font-weight:900;background:linear-gradient(135deg,#f0e8d4 0%,#d4c7a8 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;color:transparent;margin:0 0 24px;letter-spacing:-0.5px;line-height:1.15;white-space:nowrap;';hint.textContent='לחץ על האפשרות המתאימה';wrap.appendChild(hint);opts.forEach(function(o){var c=o.cloneNode(true);c.style.cssText+=';box-shadow:0 8px 24px rgba(0,0,0,0.4),0 2px 6px rgba(0,0,0,0.3)!important;border:1.5px solid rgba(240,232,212,0.35)!important;transition:all .25s ease!important;cursor:pointer;position:relative;';wrap.appendChild(c);});main.parentElement.insertBefore(wrap,main.nextSibling);var fl=document.querySelector('.cta-float');if(fl){fl.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();wrap.scrollIntoView({behavior:'smooth',block:'center'});},true);}}catch(e){}}
